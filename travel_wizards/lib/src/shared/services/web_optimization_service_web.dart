...
class WebOptimizationService {
  ...
  Future<void>? _notificationPermissionInFlight;
  ...
  Future<void> _configurePWA() async {
    ...
    // Notification permission will be requested later
    _setupNotifications();
  }
  ...
  Future<void> requestNotificationPermission({bool force = false}) async {
    if (!html.Notification.supported) return;
    if (_notificationPermissionRequested && !force) return;

    if (_notificationPermissionInFlight != null) {
      return _notificationPermissionInFlight!;
    }

    _notificationPermissionInFlight = _performNotificationPermissionRequest(
      force: force,
    );
    try {
      await _notificationPermissionInFlight;
    } finally {
      _notificationPermissionInFlight = null;
    }
  }

  Future<void> _performNotificationPermissionRequest({
    required bool force,
  }) async {
    if (!force && _notificationPermissionRequested) {
      return;
    }
