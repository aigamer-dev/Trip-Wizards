...
class NotificationService {
  ...
  /// Ensure that notification permissions are requested after login.
  Future<void> ensurePermissionsRequested({bool force = false}) async {
    ...
    _permissionInFlight = _performPermissionRequest(force: force);
    try {
  ...
  Future<void> _performPermissionRequest({required bool force}) async {
    if (!force && _permissionsRequested && !_permissionsGranted) {
      debugPrint(
        '🔔 Notification permission previously denied; skipping prompt',
      );
      return;
    }
  ...
  Future<void> _refreshMessagingState() async {
    try {
      _fcmToken = await _messaging.getToken();
      debugPrint('🔔 FCM Token: $_fcmToken');
      await _updateTopicSubscriptions();
    } catch (e) {
      debugPrint('❌ Failed to refresh messaging state: $e');
    }
  }
  ...
  Future<void> _updateTopicSubscriptions() async {
    if (!_permissionsGranted) {
      debugPrint(
        '🔔 Skipping topic subscription update until permissions granted',
      );
      return;
    }
