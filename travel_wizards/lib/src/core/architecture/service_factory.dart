...
class ServiceFactoryRegistry {
  ...
  /// Register a mock factory without generic type information
  void registerMockFactoryInstance(ServiceFactory factory) {
    _mockFactories[factory.metadata.type] = factory;
  }
  ...
  ServiceFactory? getFactoryByType(Type type) {
    if (kDebugMode && _mockFactories.containsKey(type)) {
      return _mockFactories[type];
    }
  ...
  void createAndRegisterServiceByType(
    Type type,
    EnhancedServiceLocator locator,
  ) {
    final factory = getFactoryByType(type);
    if (factory == null) {
      throw ServiceLocatorException('No factory registered for type $type');
    }

    final service = factory.create();
    locator.registerServiceByType(
      type: factory.metadata.type,
...
class ServiceLocatorBuilder {
  ...
  Future<EnhancedServiceLocator> build() async {
    ...
    for (final type in _servicesToCreate) {
      ServiceFactoryRegistry.instance.createAndRegisterServiceByType(
        type,
        _locator,
      );
    }
...
class ServiceLocatorTestUtils {
  ...
  static Future<EnhancedServiceLocator> createTestLocator({
    Map<Type, ServiceFactory<dynamic>> factoryOverrides = const {},
    List<Type> services = const [],
  }) async {
    final locator = EnhancedServiceLocator.instance;
    final registry = ServiceFactoryRegistry.instance;

    // Register mock overrides
    for (final entry in factoryOverrides.entries) {
      registry.registerMockFactoryInstance(entry.value);
    }

    final typesToCreate = <Type>{...factoryOverrides.keys, ...services};

    for (final type in typesToCreate) {
      registry.createAndRegisterServiceByType(type, locator);
    }
